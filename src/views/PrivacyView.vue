...
<main class="container flex flex-col gap-8 py-16">
    <section class="flex flex-col gap-6 text-center">
      <h1 class="text-2xl font-bold leading-[1.4]">{{ header.title }}</h1>
      <p class="text-lg leading-[1.4]">{{ header.subtitle }}</p>
      <p class="text-lg">{{ header.content }}</p>
    </section>
    <section class="flex flex-col gap-8">
      <ol class="flex flex-col gap-6">
        <li v-for="rule in rules" :key="rule.title" class="space-y-6">
          <h2 class="text-xl font-bold leading-[1.4]">{{ rule.title }}</h2>
          ...
          <div v-for="list in rule.list" :key="list">
            ...
            <ul v-else class="space-y-4">
              <li v-for="item in list.list" :key="item" class="flex items-center gap-4">
                ...
                <p class="text-lg leading-[1.4]">{{ item }}</p>
              </li>
            </ul>
          </div>
          <p v-if="rule.description" class="text-lg leading-[1.4]">{{ rule.description }}</p>
        </li>
      </ol>
    </section>
    <section class="flex flex-col gap-6">
      <h1 class="text-2xl font-bold leading-[1.4]">{{ footer.title }}</h1>
