    ...
    <div class="flex min-h-[calc(100vh-326.55px)] md:min-h-[calc(100vh-390px)]">
      <div class="px-3 basis-full">
        <div class="flex items-center justify-center h-full">
          <VForm v-slot="{ errors }" class="mx-auto min-w-96" @submit="changePassword">
            <div class="mb-6">
              <h2 class="mb-2 text-2xl font-bold md:text-4xl text-gray-1">使用者密碼變更</h2>
            </div>
            <div class="mb-4">
              ...
              <p class="mt-2 text-sm text-gray-2"> 請具備 1 個數字， 1 個大寫英文， 1 個小寫英文， 1 個特殊符號，且長度至少為 8 個字元
              </p>
              <ErrorMessage name="密碼" class="text-sm text-primary-dark"></ErrorMessage>
            </div>
            <div class="mb-4">
              <label for="confirmPassword"
                     class="block mb-2 text-base font-medium text-gray-1">再次輸入密碼</label>
              <VField id="confirmPassword" name="再次輸入密碼" :type="showPassword ? 'text' : 'password'"
                      class="block w-full p-4 text-sm bg-white border rounded-full border-gray-3 focus:ring-primary focus:border-primary disabled:bg-gray-100 disabled:border-gray-300 disabled:text-gray-2"
                      :class="{ 'is-invalid': errors[ '再次輸入密碼' ] }" placeholder="請再次輸入密碼"
                      rules="required|confirmed:@密碼" :disabled="isLoading"
            ...
            <div class="flex items-start mb-6">
              <div class="flex items-center h-5">
                <input id="showPassword" type="checkbox"
                       class="w-4 h-4 border border-gray-300 rounded text-primary bg-gray-50 focus:ring-3 focus:ring-primary-light"
                       @click="showPassword = !showPassword" :disabled="isLoading" />
              </div>
